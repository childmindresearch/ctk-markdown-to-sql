...
impl TreeToSql {
    ...
    pub fn convert_id_to_auto_increment(&mut self, table_name: &str) -> String {
        let conversion = format!(
            "
ALTER TABLE IF EXISTS public.{}
ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY ( INCREMENT 1 START {} );",
            table_name, self.next_id
        );
        self.next_id += 1;
        return conversion;
    }
    ...
    fn _write_sql_insertions_loop(
        ...
        let this_id = format!("id{}", self.next_id);
        self.next_id += 1;
        match parent_id {
            Some(parent_id) => {
                self.queries.push(format!(
                "{} AS (INSERT INTO {} (text, parent_id) VALUES('{}', (SELECT id FROM {})) RETURNING id),",
                this_id,
                table_name,
            ...
            None => {
                self.queries.push(format!(
                    "{} AS (INSERT INTO {} (text, parent_id) VALUES('{}', NULL) RETURNING id),",
                    this_id,
                    table_name,
                    self.sanitize(&tree.text),
                ));
...
fn test_write_sql_insertion() {
    ...
    assert_eq!(
        sql,
        "id1 AS (INSERT INTO templates (text, parent_id) VALUES('root', NULL) RETURNING id),\nid2 AS (INSERT INTO templates (text, parent_id) VALUES('child''s', (SELECT id FROM id1)) RETURNING id),"
    );
}
